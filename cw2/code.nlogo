...
;;
;;            ;;;; new ;;;;
;;            num-turtles-per-sub    ;; number of turtles in each subpopulation
;;            subpopulation-density  ;; the closeness of clusters
;;            ;;;; new ;;;;
;;
...
;  set knowledge-transfer 0   ; see comment about this at the top of the file.  Variable not used now.
  set num-subpopulations 4  ;; fix subpopulations to 4
end
...
to setup-agents
  ...
  (foreach (sort turtles) all-pos-x all-pos-y sub-list [
...
;; get coordinate within a quadrant
to-report get-quad-coor [grp]
  let cur-min-x item grp min-x
  let cur-max-x item grp max-x
  let cur-min-y item grp min-y
  let cur-max-y item grp max-y

  let rx-float 0
  let ry-float 0

  ; find the max abs value that a point belongs to a quadrant
  let xlim 0
  let ylim 0
  ifelse (abs cur-max-x) > (abs cur-min-x) [set xlim cur-max-x] [set xlim cur-min-x]
  ifelse (abs cur-max-y) > (abs cur-min-y) [set ylim cur-max-y] [set ylim cur-min-y]

  ; find a random position wihtin boundary
  set rx-float random-float xlim
  set ry-float random-float ylim

  while [rx-float = 0 or (not (rx-float >= cur-min-x)) or (not (rx-float <= cur-max-x))] [
    set rx-float random-float xlim
  ]
  while [ry-float = 0 or not (ry-float >= cur-min-y) or not (ry-float <= cur-max-y)] [
    set ry-float random-float ylim
  ]

  report (list rx-float ry-float)
end

to set-quads
  ; setting quardrants
  set min-x (list -60  3 -60   3)
  set max-x (list -3  60  -3  60)
  set min-y (list  3   3 -60 -60)
  set max-y (list 60  60  -3  -3)
end
...
to set-pos
  ...
  set sub-list n-values num-agents [0]

  (foreach min-x max-x min-y max-y [
      let cur-min-x ?1
      let cur-max-x ?2
      let cur-min-y ?3
      let cur-max-y ?4
      ...
      foreach n-values num-turtles-per-sub [ (? - 1) + 1 ] [
        ; add to subpopulation list
        set sub-list replace-item ctr sub-list grp
        let pt get-quad-coor(grp)

        ; add initial position to all-pos-<>
        set all-pos-x replace-item ctr all-pos-x (item 0 pt)
        set all-pos-y replace-item ctr all-pos-y (item 1 pt)
        set ctr (ctr + 1)
      ]
      ;;increment grp
      set grp (grp + 1)
...
to-report migrate-or-not
    if (migration?) [ifelse (1 = (random (10 ^ freq-of-migration))) [report true] [report false]]
end
...
to move-somewhere
  ...
  ;move-forward
  ifelse (migration?) [ifelse migrate-or-not [move-migrate] [move-within-quad]] [move-within-quad]
end

;; migrate to another quad
to move-migrate
  let tmp-quad quad  ;; for testing
  let new-quad random 4
  let new-coor get-quad-coor new-quad
...
to-report get-to-walk
  let dist-x 0
  let dist-y 0
  ifelse (random 2 = 0) [set dist-x run-dist] [set dist-x (- run-dist)]
  ifelse (random 2 = 0) [set dist-y run-dist] [set dist-y (- run-dist)]
  let to-walk-x random-float dist-x
  let to-walk-y random-float dist-y

  report (list to-walk-x to-walk-y)
end

;; to move around within subpopulation
to move-within-quad
  ; initial distance to walk and set pos or neg randomly
  let to-walk get-to-walk

  ; set initial new position to go to
  let new-x (xcor + item 0 to-walk)
  let new-y (ycor + item 1 to-walk)

  ; get boundary values
  let cur-min-x item quad min-x
  let cur-max-x item quad max-x
  let cur-min-y item quad min-y
  let cur-max-y item quad max-y
  ...
  ; find out if possible otherwise try to change the new position
  while [not (new-x >= cur-min-x) or not (new-x <= cur-max-x) or not (new-y >= cur-min-y) or not (new-y <= cur-max-y)] [
    set to-walk get-to-walk
    set new-x (xcor + item 0 to-walk)
    set new-y (ycor + item 1 to-walk)
  ]
